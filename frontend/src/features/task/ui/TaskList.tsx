...
export function TaskList(props: TaskListProps) {
  ...
  return (
    <Reorder.Group axis="y" values={props.tasks} onReorder={(tasks) => props.canReorder && props.onReorder?.(tasks.map((task) => task.id))} className="w-full space-y-2 p-0">
      {props.tasks.map((task) => <TaskRow key={task.id} task={task} canReorder={props.canReorder} showCompany={props.showCompany} showReorder={props.showReorder} onToggle={props.onToggle} onEdit={props.onEdit} onDelete={props.onDelete} />)}
    </Reorder.Group>
  );
}
